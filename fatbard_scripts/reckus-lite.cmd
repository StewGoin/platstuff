...
### If you want to recall while playing set these variables (yes/no)
	var immortal yes
	var herb yes
	var famous yes
	var holiday yes
...
vocaltest:
	gosub clear
	if "%vocals-yes-or-no" = "no" then goto instruments
...
instruments:
	gosub clear
	if ("%first" != "none") then gosub %first
	gosub clear
	if ("%second" != "none") then gosub %second
	gosub clear
	if ("%third" != "none") then gosub %third
	gosub clear
	goto finished
...
windtest:
	...
	if "%wind-instrument" = "no" then return
	var diffnum $diffWI
...
stringtest:
	...
	if "%string-instrument" = "no" then return
	var diffnum $diffSTR
...
perctest:
	...
	if "%percu-instrument" = "no" then return
	var diffnum $diffPCN
	var instrument %percu-instrument
	var mskill PCN
	var xskill Percussions
	gosub begin
...
play:
	...
	match playwait You are already
...
playinstrument:
	...
	if "%immortal" = "yes" && $Scholarship.LearningRate < 34 then
	{
		...
		waitforre Your memory seems a bit too clouded|You recall|you don't seem to recall|You don't recall anything
	}
	else pause 14
	pause
#	gosub earlycheck
