...
var TRAIN_LOCATION FANG|SHEL|CROSSING|RIVERHAVEN|SHARD|AIN|CROSSLUMBER|THERENLUMBER
#|LETH|KRESH
var TRAIN_LOCATION_MAX 7
...
if contains("$scriptlist", "geniehunter.cmd") then goto HUNT_CYCLE_CHECK
...
ORIGAMI:
  if ("$lefthandnoun" <> "") then gosub put my $lefthandnoun in my $pack
  if ("$righthandnoun" <> "") then gosub put my $righthandnoun in my $pack
  if $Mechanical_Lore.LearningRate < 31 then
  {
    put .origami
...
room_check:
  var home_rooms 0
  var hib_rooms 75|159|163|164|193|218|298|320|386|407|409|410|412|418|116|96|287|411
  var crossing_rooms 51|487|488|476|146
  ...
  var shardwest_rooms 10|15|37|14|391|395|460|481
...
HUNT_PREP_SUPPLIES:
  ...
  gosub BUNDLE_ROPE
HUNT_PREP_BANK:
  gosub go_room premium exchange
  put excd
  pause 3
  put depall
  pause 2
...
shardwest_395:
  ...
  if $Performance.LearningRate > 30 then goto HUNT_FIGHT_PREP
  pause
  put unlock chateau;open chateau;go chateau
ryken_home:
home_0:
  gosub ALMANAC_CHECK
  if $Performance.LearningRate < 31 then gosub reckus
  gosub ALMANAC_CHECK
  if $Locksmithing.LearningRate < 32 then gosub locks
  gosub ALMANAC_CHECK
  #if $Mechanical_Lore.LearningRate < 31 then gosub origami
  put unlock door;open door;go door
  pause
HUNT_FIGHT_PREP:
shardwest_391:
  # fight prep
  if $roomid != 391 then gosub go_room 391
  gosub ALMANAC_CHECK
  put #var train_hunt 0
  put .fight reprep soul all
...
shardwest_15:
  ...
  if $SpellTimer.AbandonedHeart.active = 1 then put release cyclic
HUNT_DONE_RETURN:
  # return to hib
  if $SpellTimer.AbandonedHeart.active = 1 then put release cyclic
  gosub go_room shard
  if "$zoneid" != "67" then gosub go_room shard
  put .ptravel HIBARN
  waitfor Done Portalhax
HUNT_DONE_WOUNDS:
  # check wounds
...
GEM_SELL:
  ...
  CARD_CLEAN_1:
    ...
    put get my card;card add;put card in bucket
    matchwait 2
    goto CARD_CLEAN_1
  CARD_CLEAN_2:
  ...
  DIRA_CLEAN_1:
    ...
    put get my dira;dira add;put dira in bucket
    matchwait 2
    goto DIRA_CLEAN_1
  DIRA_CLEAN_2:
...
BUNDLE_ROPE:
  ...
  BUNDLE_ROPE_1:
    ...
    goto BUNDLE_ROPE_1
  BUNDLE_ROPE_ASK:
    put ask vandorf for bundling rope
    pause 0.5
...
hib_BUNDLE:
	...
	if ("$righthandnoun" <> "") then gosub put my $righthandnoun in my $pack
 	pause
	put get wyvern.claw from my bundle
 	pause
 	put stow wyvern.claw
 	pause
...
# if ($Engineering.LearningRate > 25) then goto hib_407
...
hib_407:
  ...
  PACKET_CHECK_BUY:
    if $roomid != 415 then gosub go_room 415
    put .order 16 1
    waitfor All bought!
    goto PACKET_CHECK_0
...
### Merkresh
# Combat (Primary & Backtrain?)
# do combat circuit
#return to hib for heal/sell, etc...
...
kresh_106:
  ...
  pause
  gosub health_check
  if %wounded = 1 then gosub kresh_HEALER
  put #echo >Log DarkBlue,LightYellow Merkresh back to Hib.
  put .ptravel  HIBARN
  waitfor Done Portalhax
  goto HIB_ITEM_REPAIR
