...
put #var GH_SETUP 1
put #var GH_NOHUNT 1
if "$GH_HUNT_TIME" = "" then put #var GH_HUNT_TIME $time
...
VARIABLES_INIT:
	...
	var junkloot ((orichalcum|animite|haralun|damite|kertig|glaes|audrualm|niello|tyrium|vardite|silversteel|icesteel) (nugget|shard|tear|fragment|lump))|silver nugget|platinum nugget|gold nugget|scroll|tablet|card|vellum|sheiska leaf|ostracon|hhr'lav'geluhh bark|papyrus roll|smudged parchment|naphtha|\bbolts?\b|\bdira\b|flarmencrank|\bgear\b|glarmencoupler|kirmhiro draught|\bmap\b|\bnuts?\b|package|rackensprocket|soulstone|spangleflange|vardite hinges|mistsilk padding|ilomba boards|frothy-green oil|dragon pendant|seastars?
	...
	var monsters7 faenrae assasin|telga moradu|trekhalo|orc clan chief|shadow master|sky giant|imp|dummy|pile of rubble|Priest crone|sentinel|zealot|assassin|juggernaut|gam chaga|ashu hhinvi|small sand dune|beltunumshi
	...
	var skinnablemonsters8 shadow beast|cinder beast|asaren celpeze|spirit dancer|scaly seordmaor|poloh'izh|armadillo|shalswar|hierophant|black ape|Isundjen conjurer|wyvern
	...
	var skins wyvern claw|celpeze eye

	var OPTIONVARS \b(AMB|APPR|ARRA|BS|BACK|BLOC|BUN|BRA|COLL|CYC|EMP|DEB|DOD|EVA|EXP|LOOT|OFF|PAR|POA|SING|CHANT|NORET|SKI|SNA|STAC|THR|TI|MSNA|TM|PM|MAG|TR|TAR|TAC|DSET|DEF|MSET|MU|DMSET|DMU|SWAP|BAST|MECH|GROUP|MDOPTSET|WO|NOPOACH|ARRA|HELP|BOB|MDOPT|DYING|RET|FOLD|ORIG|TYPE|SLOW|RPA|HIDING|CAM|REC|BLA|SOR|RES|RCY|BUF|NORM|LOB|MAF|HUN|FLE|IGN)(.+)?
...
BARD:
  ...
  shift
  if $SpellTimer.%BUFFSPELL.active = 0 then
  {
...
#		if (("$GH_CAMB" = "ON") && (($cam_slipped = 1) || ($cam_leeched = 1)) then gosub CAM_COMBAT
    ...
    {
      action remove ^\.\.\.wait|^Sorry, you may only type
      put .cast resonance 10 45 45 %CURR_WEAPON
      waitfor Spell Cast
      action goto PAUSE when ^\.\.\.wait|^Sorry, you may only type
    }
    if $GH_IGNITE = 1 then
...
NO_MONSTERS:
  ...
  gosub ALMANAC_CHECK
	...
	#debuglevel 5
	if (($GH_HUNT = 1) && ($Perception.LearningRate < 32) && ($gametime >= $GH_HUNT_TIME)) then gosub GH_HUNTING
	#debuglevel 0
	if ("%RANGED" = "ON") then
	{
...
#  if ($SpellTimer.LayWard.active = 0 || $SpellTimer.LayWard.duration < 3) then
#  {
#	put .cast lw 10 48
#	waitfor Spell Cast
#  }
...
GH_HUNTING_1:
	var GHHUNTTIME $GH_HUNT_TIME
	evalmath GHHUNTTIME (%GHHUNTTIME + 80)
	put #var GH_HUNT_TIME %GHHUNTTIME
...
RANGE_LOAD:
	put #var GH_NOHUNT 0
	var LAST RANGE_LOAD
	if matchre("$roomobjs", "\b%AMMO\b|basilisk head arrow|thin steel-tip bolt|stone shard|ice-adder bolt|indurium sphere|bone bolt|chakram") then
	{
		pause 0.5
...
#	put #var GH_NOHUNT 1
	...
	gosub clear
	if matchre("$lefthand", "%AMMO|basilisk head arrow|thin steel-tip bolt|stone shard|ice-adder bolt|indurium sphere|bone bolt|chakram") then put stow my %AMMO
...
SKIN_COUNT:
	if %skincount = 200 && (matchre("$righthand", "(%skins)") || matchre("$lefthand", "(%skins)")) then
	{
		var dropskin $1
		pause 0.5
		put drop %dropskin
		...
		pause 0.5
		put put my tight.bundle in my pale.pouch
		pause 0.5
		put get my bund.rope;get %dropskin;bundle
		pause 0.5
		put wear my bundle
		pause 0.5
		if ("%SHIELD" != "NONE") && ("%WORN" = "OFF") then
...
SKIN_FAIL:
	...
	# echo
	if matchre("$righthandnoun", "(eye)") then
	{
...
LOOT_OFF:
	...
	if contains("$roomobjs", "chakram") then gosub THROWN_CLEAN chakram
...
LOOT_ON:
	...
	if contains("$roomobjs", "chakram") then
	{
		gosub THROWN_CLEAN chakram
		goto LOOT_BLADE_CHECK
	}
	if matchre("$roomobjs", "an (ice-adder bolt)|a senci (stone shard)|a matte indurium (sphere)") then
	{
...
TIE_POUCH:
	var LAST TIE_POUCH
	if matchre("$lefthand", "(%gems1|%gems2|%gems3|%gems4)") then
	{
		put empty left
		waitforre You drop|What were you
	}
	if matchre("$righthand", "(%gems1|%gems2|%gems3|%gems4)") then
	{
		put empty right
		waitforre You drop|What were you
	}
...
SWITCH_WEAPON_LAST_CHECKS:
	if matchre("$roomobjs", "(light kertig throwing axe)") then
	{
		gosub THROWN_CLEAN throwing.axe
		goto SWITCH_WEAPON_LAST_CHECKS
	}
	if matchre("$roomobjs", "(chakram)") then
	{
		gosub THROWN_CLEAN chakram
		goto SWITCH_WEAPON_LAST_CHECKS
	}
	if matchre("$roomobjs", "(senci stone shard)") then
	{
		gosub THROWN_CLEAN stone.shard
		goto SWITCH_WEAPON_LAST_CHECKS
	}
    if matchre("$roomobjs", "(matte indurium sphere)") then
	{
		gosub THROWN_CLEAN sphere
		goto SWITCH_WEAPON_LAST_CHECKS
	}
	if matchre("$roomobjs", "(icc-adder bolt)") then
	{
		gosub THROWN_CLEAN ice-adder.bolt
		goto SWITCH_WEAPON_LAST_CHECKS
	}
	if matchre("$roomobjs", "(cuska)" then
	{
		gosub THROWN_CLEAN cuska
		goto SWITCH_WEAPON_LAST_CHECKS
	}
	if matchre("$roomobjs", "(throwing mallet)") then
	{
		gosub THROWN_CLEAN throwing.mallet
		goto SWITCH_WEAPON_LAST_CHECKS
	}
