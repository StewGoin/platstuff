...
VARIABLES_INIT:
	...
	var OPTIONVARS \b(AMB|APPR|ARRA|BS|BACK|BLOC|BUN|BRA|DAN|COU|COLL|CYC|EMP|DOD|EVA|EXP|LOOT|OFF|PAR|POA|SING|CHANT|NORET|SKI|SNA|SNI|STAC|STAN|THR|TI|MSNA|TM|PM|MAG|TR|TAR|TAC|CUST|DSET|DEF|MSET|MU|DMSET|DMU|SWAP|BAST|NOPAR|NOEVA|NOSHI|MECH|JUG|YOYO|GROUP|MDOPTSET|WO|FLEE|NOPOACH|ROAM|ARRA|HELP|BOB|COM|PERC|MDOPT|HUNT|DYING|RET|FOLD|ORIG|TYPE|SLOW|RPA|HIDING|CAM|REC|BLA|SOR|RES|RCY|BUF)(.+)?
...
#		if (("$GH_CAMB" = "ON") && (($cam_slipped = 1) || ($cam_leeched = 1)) then gosub CAM_COMBAT
    ...
    if %RESONANCE = 1 then
    {
      var casttype %CURR_WEAPON
      gosub PREP_SPELL resonance $resonance_mana
    }
    var casttype ""
    gosub SPELLCHECKING
		if ("%MAGIC_TYPE" != "OFF") then var MAGIC_COUNT %c
...
SPELLCHECKING:
  put #var GH_NOHUNT 1
  if ((%SORC_PREPPED = 1) && (%SORC_READY = 1)) then gosub SORC_CAST
  if (("$GH_BUFF" = "ON") && ("$GH_CYCLIC" = "OFF") && ($spell_$GH_BUFF_SPELL_isactive = 0)) then
  {
    ...
    put .cast $GH_BUFF_SPELL $GH_BUFF_MANA
    waitfor Spell Cast
  }
  if $spell_dots_isactive = 0 then
  {
    if (%SORC_PREPPED = 1) && (%SORC_READY = 0)) then
    {
      pause 0.5
  ...
  if $spell_harmony_isactive = 0 then
  {
    ...
    put .cast harmony 10 40 40
    waitfor Spell Cast
  }
  if $spell_not_isactive = 0 then
  {
    if (%SORC_PREPPED = 1) && (%SORC_READY = 0)) then
    {
      pause 0.5
    ...
    put .cast not 10 40 50
    waitfor Spell Cast
  }
  if $spell_rotc_isactive = 0 then
  {
    if (%SORC_PREPPED = 1) && (%SORC_READY = 0)) then
    {
      pause 0.5
...
PREP_SPELL:
  ...
  var SPELLTYPE SPELL
  	matchre PREP_CAMB You trace an angular sigil in the air, shaping the pattern|fully prepared|already preparing|Tiny tendrils of lightning jolt between your hands|You rock back and forth
...
PREP_CAMB_1:
	...
	matchre PREP_CAM_FOCUS absorbs|dissipates|cannot harness
...
PREP_CAM_FOCUS:
	...
	matchre PREP_WAIT Something in the area|Something is damping
	put focus my $cambrinth_spell spell
	matchwait 2
	goto CAM_FOCUS
...
SORC_PREP:
  if $mana < 40 then return
  	matchre SORC_RETURN You trace an angular sigil in the air, shaping the pattern|fully prepared|already preparing|Tiny tendrils of lightning jolt between your hands|You rock back and forth
...
SORC_CAST:
    action (sorc) off 
		matchre SORC_CAST_RETURN You gesture|you launch into the introduction|you begin to sing|you begin to chant|hummed tones form a soft backdrop for the opening notes|you begin the opening passages|You begin to sing|You hum the low|In a low tone you begin your enchante|then begin to chant|take shape in your mind|you launch yourself into the introduction of|As you begin to sing|you soon allow your voice to dance along the darkly melodic introduction to Abandoned Heart|You briefly close your eyes to prepare yourself to begin the Phoenix's Pyre|You lift your right hand, forming your fingers into the shape of a vulture's talons.|You don't have a spell prepared
...
BOBBING:
    var BOBCOUNTER 0
		...
		matchre RETURN Roundtime|At what are you trying to|You aren't close enough to attack|^There is nothing else to face\!|^What are you trying to attack\?
	put bob
		matchwait 5
		goto BOBBING
...
400:
	...
	counter add 10
	save 400
	if (("$GH_BOB" = "ON") && ("$GH_AMBUSH" = "OFF") && ($Tactics.LearningRate < 34)) then var COMMAND bob
...
#		match RANGE_RETREAT melee range on you  
		...
		if ($Tactics.LearningRate < 34) then put bob;-1.5 circle
	matchwait	
1040:
RANGE_FIRE:
	var LAST RANGE_FIRE
...
#	put #statusbar 1 GH - Kills:$GH_KILLS  Skins:$GH_SKINS  Loots:$GH_LOOTS
	if (toupper("$GH_APPR") = "ON") || ("%DEAD" = "YES")) then goto APPR_$GH_APPR
...
SWITCH_WEAPON:
  ...
  if (%SORC_PREPPED = 1) then
  {
	...
	if "%CYCLIC" = "ON" then
	{
	  pause
	  put rel cyclic
	  gosub PREP_CYCLIC desert maelstrom 30
	  pause
...
SWITCH_WEAPON_LAST_CHECKS:
	if matchre("$roomobjs", "(cuska|throwing mallet|hand mallet|tempered pitchfork|elongated stone)" then
	{
