...
action put #queue clear;var Action clamp when with clamps or a vice to hold it in place|ready to be clamped together|compressed with clamps
...
excess:
	pause
	 ...
	 send get %excessmats
	 wait
	 send put my %excessmats in my %engineering.storage
	 wait
	 goto work
...
ToolStow:
	...
	else send put my $righthandnoun in my %engineering.storage
...
arrowhead_make:
	send shape %1
	pause 5
	send put my arrowh in my %engineering.storage
	pause 1
	...
	if %stain.gone = 1 then
		{
		 ...
		 send put my stain in my %engineering.storage
		 waitforre ^You put
		 if ("$righthandnoun" != "$MC.order.noun" && "$lefthandnoun" != "$MC.order.noun") then send get my $MC.order.noun from my %engineering.storage
		 var stain.gone 0
		}
	if %glue.gone = 1 then
		{
		 ...
		 send put my glue in my %engineering.storage
		 waitforre ^You put
		 if ("$righthandnoun" != "$MC.order.noun" && "$lefthandnoun" != "$MC.order.noun") then send get my $MC.order.noun from my %engineering.storage
		 var glue.gone 0
		}
