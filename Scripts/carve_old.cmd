...
if ("%1" = "") then
{
	echo USAGE .carve <item crafted> <metal for ingot> (tempered yes|no) (balance|hone) (number to make for workorder)
	exit
}

var BookType carving
#MAGICS/MECHS
  var magic 1
  var origami 1
  if ("%1" = "quick") then
...
#  put carve %MaterialSize with $saw
  put carve %Material with $chisel
  matchwait 5
  goto FIRST_CHISEL
...
CARVE_IT:
  ...
  match NEED_POLISH Upon finishing you see some discolored areas on
  match CARVE_IT Roundtime
  put carve $engineer_noun with $chisel
  matchwait 5
  goto CARVE_IT
...
NEED_RIFFLERS:
  if %ITEM_COMPLETE = 1 then goto ASSEMBLE_IT
  gosub put my $chisel in my $craftpack
  gosub get my $riffler
RIFFLE_IT:
  match ASSEMBLE_IT [Ingredients
  match ITEM_FINISHED Applying the final touches, you complete working
  match GET_CHISEL Roundtime
  put rub $engineer_noun with $riffler
  matchwait 5
  goto RIFFLE_IT
...
RASP_IT:
  ...
  match ITEM_FINISHED Applying the final touches, you complete working
  match GET_CHISEL Roundtime
  put scrape $engineer_noun with $rasp
  matchwait 5
  goto RASP_IT
...
POLiSH_IT:
  ...
  match ITEM_FINISHED Applying the final touches, you complete working
  match GET_CHISEL Roundtime
  put apply surface.polish to $engineer_noun
  matchwait 5
  goto POLISH_IT
